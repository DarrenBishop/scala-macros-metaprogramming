...
object ReflectionBasics {
  ...
  private def callMethodDynamicallyImpl[A: Type](instance: Expr[A], arg: Expr[Int], methodName: Expr[String])(using q: Quotes): Expr[String] = {
    // import the reflection package
    import reflects.*

    // Term = loosely typed Expr = piece of an AST
  ...
  def createTupleImpl[N <: Int: Type, A: Type](value: Expr[A])(using Quotes) = {
    // import the reflection package
    ...
    def buildTupleComplicated_aux(tpe: Type[? <: AnyKind]): Expr[Tuple] =
      tpe match {
        case '[A *: rt] =>
          '{ $value *: ${ buildTupleComplicated_aux(TypeRepr.of[rt].asType) } }
        case _ => '{ EmptyTuple }
      }
    ...
    inline def buildTupleComplicated(n: Int): Expr[Tuple] = {
      ...
      // 1 - build the type constructor
      val tupleConstructor = defn.TupleClass(n).typeRef // as `isType` returns true
